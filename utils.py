...
# Convert the TR
def label2TR(stim_label_allruns, num_runs, TR, TRs_run, events_run):
...
def blockwise_sampling(eventwise_data, eventwise_labels, events_per_block=10):
    ...
    number_of_voxels = eventwise_data.shape[1]
    reshaped_data = eventwise_data.reshape((events_per_block, expected_blocks * number_of_voxels), order='F')
    mean_reshaped_data = np.mean(reshaped_data, axis=0)
    blockwise_data = mean_reshaped_data.reshape((expected_blocks, number_of_voxels), order='F')
